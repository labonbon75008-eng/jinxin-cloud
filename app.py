...
def execute_code_and_clean(txt):
        ...
        try:
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                exec(safe_code, globals(), {
                    ...
                    'plt':plt, 'pd':pd, 'yf':yf, 'datetime':datetime, 'contextlib':contextlib
                })
            if plt.get_fignums():
    ...
    # 2. 清洗文本：彻底删除代码块
    clean_txt = re.sub(r'```.*?```', '', txt, flags=re.DOTALL).strip()
    return clean_txt, img_path
...
if text_input:
    ...
    # 使用简单的时间戳判断，防止重复
    # 注意：mic_recorder 每次录音结束都会触发 rerun，这里直接识别
    ...
    with st.chat_message("assistant", avatar=get_avatar()):
        with st.spinner("思考中..."):
            try:
                ...
                # 清洗代码 + 执行画图
                clean_txt, img_p = execute_code_and_clean(raw_txt)
