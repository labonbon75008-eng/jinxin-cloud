...
# 【核心修复】语音组件防崩导入：如果环境不支持，直接置空，不报错
try:
...
try:
    with c_voice:
        if mic_recorder:
            # key 必须固定，防止刷新丢失
            audio_val = mic_recorder(start_prompt="🎙️", stop_prompt="⏹️", key='mic_recorder')
            if audio_val and audio_val['bytes']:
                # 简单防抖
                if audio_val['id'] != st.session_state.last_audio:
                    st.session_state.last_audio = audio_val['id']
                    with st.spinner("识别中..."):
                        audio_result = transcribe(audio_val['bytes'])
                        if not audio_result: st.toast("语音未识别，请重试")
        else:
            st.error("语音模块未加载")
except Exception as e:
    st.error("语音组件崩溃，请使用文字")
