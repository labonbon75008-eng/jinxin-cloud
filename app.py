...
# API KEY 读取机制 (兼容本地和云端)
# 1. 优先尝试 Streamlit Secrets (云端)
# 2. 失败则提示用户配置
try:
    API_KEY = st.secrets["GEMINI_API_KEY"]
except:
    st.error("🚨 未配置 API Key！请在 Streamlit 后台 Settings -> Secrets 中添加：GEMINI_API_KEY = '您的Key'")
    st.stop()
...
def get_stock_data_v8(ticker_symbol):
    """V8.0 极速数据引擎：新浪实时 + Yahoo历史"""
    sina_code = get_sina_code(ticker_symbol)
    
    # 1. 抓实时价 (Sina HTTP) - 极速，不走代理
    info_str = "暂无实时数据"
    current_price = 0.0
    try:
    ...
    try:
        ...
        hist = ticker.history(period="1mo") # 拿1个月数据
        if not hist.empty:
            df = hist[['Close']]
    except:
        # 如果Yahoo挂了，造一个伪数据防止画图报错
        if current_price > 0:
...
def execute_local_code_and_save(code_str):
    ...
    try:
        ...
        # 【关键修复】将 plt, pd, yf 等库注入到局部变量中，AI 才能用
        local_vars = {
...
# 这里的 ai_avatar 是用来给 st.image 用的，如果是 None 就不能传给 st.image
...
for i, msg in enumerate(st.session_state.messages):
    ...
    if not current_avatar: # 如果是 None, 用默认 emoji
